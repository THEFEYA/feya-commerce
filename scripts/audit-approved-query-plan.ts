...
const [plan, source, content, storefront, metadata] = texts.map(t => JSON.parse(t));
...
const rows = content.products.map((draft: any) => {
  ...
  return { canonical_product_id: draft.canonical_product_id, seo_page_id: product.seo_page_id,
    ...
    approval_preserved: true, catalog_target_preserved: true,
  };
});
...
const candidateEvidence = plan.page_candidates.map((candidate: any) => {
  ...
  const matches = codes ? offers.filter(r => r.offer.status === 'ready' && r.offer.atomic_options.some(o => codes.includes(o.code))).map(r => r.id) : null;
  return { ...candidate, supporting_cluster_codes: clusters.filter((c: any) => c.proposed_owner_candidate_code === candidate.candidate_code).map((c: any) => c.proposal_code),
    ...
    meaning: 'Configuration matches within the 208-product target only; not page eligibility or approved inventory membership.' };
});
const report = { contract_version: 'approved_catalog_query_audit_v1', mode: 'offline_proposal_not_registered',
  ...
  implementation_fingerprints: ['scripts/audit-approved-query-plan.ts', 'lib/storefrontSellableOffer.ts', 'lib/storefrontOwnerReviewedCorrections.ts', 'lib/storefrontReconciledOfferCorrections.ts']
