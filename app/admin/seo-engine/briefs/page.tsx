      ...
      {brief ? <>
        ...
        <div className="grid xl:grid-cols-[.9fr_1.1fr] gap-5 mb-5">
          ...
          <Panel title="Релевантные ключи первой волны" icon={UploadCloud}>
            ...
            {brief.rejectedKeywords?.length ? <div className="mt-4 border-t border-[rgba(216,214,211,.10)] pt-3"><div className="text-[10px] uppercase tracking-[0.18em] text-[var(--smoke)] mb-2">Отброшено как нерелевантное</div><div className="flex flex-wrap gap-1.5">{brief.rejectedKeywords.slice(0, 8).map((keyword, index) => <span key={`${keyword.keyword_norm || keyword.keyword}-rejected-${index}`} className="rounded-full border border-[rgba(196,64,88,.25)] bg-[rgba(160,32,56,.06)] px-2 py-1 text-[10px] text-[var(--bone-dim)]">{keyword.keyword || keyword.keyword_norm} · {keyword.pilot_relevance_reason}</span>)}</div></div> : null}
          </Panel>
        </div>
