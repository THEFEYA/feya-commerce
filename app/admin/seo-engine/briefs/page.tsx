...
async function loadPilotData() {
  ...
  if (!supabase) return { product: null, keywords: [], error: getMissingSupabaseEnvMessage() };

  const [productsResult, keywordsResult] = await Promise.all([
    supabase.from(STOREFRONT_VIEW_V4).select(STOREFRONT_V4_CARD_SELECT).limit(30),
    supabase.from('feya_commerce_v_seo_keyword_ai_cleanup_report_v1').select(KEYWORD_SELECT).limit(500),
  ]);

  const error = productsResult.error?.message || keywordsResult.error?.message || null;
  const products = productsResult.data || [];
  const keywords = keywordsResult.data || [];
  const product = products.find((row) => row.primary_image_url && row.product_slug && (row.category_label || row.product_type)) || products[0] || null;

  return { product, keywords, error };
}
...
export default async function SeoEngineBriefsPage() {
  const { product, keywords, error } = await loadPilotData();
    ...
    <section className="container-feya pt-10 pb-16">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between border-b border-[rgba(216,214,211,.12)] pb-7 mb-7">
        <div>
          ...
          <p className="mt-4 max-w-3xl text-[15px] leading-relaxed text-[var(--bone-dim)]">Контролируемый read-only pilot: один товар, реальные product facts, tier-1 keyword candidates и gate качества. Ничего не записывается в Supabase и ничего не публикуется.</p>
        </div>
      ...
      {error ? <div className="rounded-2xl border border-[rgba(212,178,106,.30)] bg-[rgba(212,178,106,.07)] p-5 text-[13px] leading-relaxed text-[var(--bone-dim)] mb-7">Ответ базы: {error}</div> : null}
      {!brief ? <div className="rounded-2xl border border-[rgba(196,64,88,.35)] bg-[rgba(160,32,56,.10)] p-5 text-[var(--bone-dim)]">Не удалось собрать pilot: нет товара из storefront view.</div> : null}
        ...
        <div className="grid lg:grid-cols-[1fr_360px] gap-6 mb-6">
          <Panel title="Товар для пилота" icon={FileText}>
            <div className="grid md:grid-cols-[130px_1fr] gap-5 items-start">
              <div className="relative h-40 rounded-xl overflow-hidden bg-black/30 border border-[rgba(216,214,211,.10)]">{product.primary_image_url ? <img src={product.primary_image_url} alt="" className="absolute inset-0 h-full w-full object-cover" /> : null}</div>
              <div>
