...
export default async function SeoBriefsFromDecisionPage({ searchParams }) {
  ...
  const brief = data.product ? buildSeoPilotBrief(data.product, data.keywords) : null;
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_80%_0%,rgba(212,178,106,.13),transparent_32%),linear-gradient(180deg,#07070A,#111016_45%,#07070A)]">
    <section className="container-feya pt-7 pb-12">
      <div className="grid gap-5 lg:grid-cols-[1fr_460px] lg:items-end border-b border-[rgba(216,214,211,.12)] pb-6 mb-6">
        <div><div className="eyebrow-gold mb-2">Админка · SEO-задание</div><h1 className="font-tall text-bone leading-none" style={{ fontSize: 'clamp(42px,6vw,72px)' }}>SEO-задание товара</h1><p className="mt-3 max-w-3xl text-[14px] leading-relaxed text-[var(--bone-dim)]">Этот экран берёт сохранённый черновик из Мастера листинга: товар, полный фокус, режим и выбранные ключи. Здесь проверяем основу перед генерацией SEO-текста.</p></div>
        <div className="flex flex-wrap gap-3 lg:justify-end"><Link href="/admin/listing-master" className="btn-ghost">Мастер листинга <ArrowUpRight size={13} /></Link><Link href="/admin/seo-keywords" className="btn-ghost">SEO-ядро <ArrowUpRight size={13} /></Link>{data.product?.product_slug ? <Link href={`/shop/${data.product.product_slug}`} className="btn-ghost">Открыть товар <ArrowUpRight size={13} /></Link> : null}</div>
      </div>
      ...
      {data.product ? <div className="grid xl:grid-cols-[.95fr_1.05fr] gap-5 mb-5">
        <Panel title="Товар" icon={FileText}>
          <div className="grid sm:grid-cols-[118px_1fr] gap-4"><div className="h-32 rounded-xl overflow-hidden border border-[rgba(216,214,211,.10)] bg-black/30">{data.product.primary_image_url ? <img src={data.product.primary_image_url} alt={data.product.primary_image_alt || data.product.card_title || ''} className="h-full w-full object-cover" /> : <div className="h-full flex items-center justify-center text-[10px] uppercase tracking-[0.18em] text-[var(--smoke)]">нет фото</div>}</div><div><h2 className="text-bone text-[18px] leading-tight">{titleOf(data.product)}</h2><div className="mt-2 text-[11px] text-[var(--bone-dim)]">/{data.product.product_slug || 'no-slug'}</div><div className="mt-4 grid sm:grid-cols-2 gap-2"><Fact label="Раздел" value={data.product.operator_section_label || data.product.category_label} /><Fact label="Источник Etsy" value={data.product.source_category_label || '—'} /><Fact label="Материал" value={data.product.material || '—'} /><Fact label="Цвет" value={data.product.canonical_color_label || data.product.color || '—'} /><Fact label="World" value={data.product.world_label || '—'} /><Fact label="ДНК-риск" value={data.product.has_component_review_risk ? `проверить: ${data.product.needs_component_review_count || 0}` : 'нет'} /></div></div></div>
        </Panel>

        <Panel title="Решение из Мастера листинга" icon={CheckCircle2}>
          {data.decision ? <div className="space-y-3"><div className="flex flex-wrap gap-2"><Chip tone="success">черновик сохранён</Chip><Chip tone="gold">{STRATEGY_LABELS[data.decision.selected_strategy] || data.decision.selected_strategy || 'режим не указан'}</Chip><Chip>{data.keywords.length} ключей</Chip></div><div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2"><Fact label="Состав" value={data.manualFocus.component || 'не выбран'} /><Fact label="Материал / цвет" value={data.manualFocus.material || 'не выбран'} /><Fact label="Событие" value={data.manualFocus.event || 'не выбрано'} /><Fact label="Стиль" value={data.manualFocus.style || 'не выбран'} /><Fact label="Персона / образ" value={data.manualFocus.persona || 'не выбрана'} /><Fact label="Аудитория" value={data.manualFocus.audience || 'не выбрана'} /></div><div><div className="eyebrow-dim mb-2">Ключи из черновика</div><div className="max-h-[220px] overflow-auto rounded-xl border border-[rgba(216,214,211,.10)] divide-y divide-[rgba(216,214,211,.08)]">{data.keywords.slice(0, 30).map((kw, i) => <div key={`${kw.keyword_norm || kw.keyword}-${i}`} className="grid grid-cols-[1fr_90px_90px] gap-2 px-3 py-2 text-[11px]"><div className="text-bone">{kw.keyword || kw.keyword_norm}</div><div className="text-[var(--bone-dim)]">{kw.avg_monthly_searches ?? '—'}</div><div className="text-[var(--gold-warm)]">{kw.competition || '—'}</div></div>)}</div></div></div> : <div className="text-[13px] text-[var(--bone-dim)]">Нет сохранённого решения.</div>}
        </Panel>
      </div> : null}

      {brief ? <>
        <div className="grid xl:grid-cols-[.85fr_1.15fr] gap-5 mb-5">
          <Panel title="Проверки качества" icon={ShieldAlert}><div className="space-y-2">{brief.blockerChecks.map((check) => <div key={check.label} className="grid grid-cols-[120px_92px_1fr] gap-2 items-center rounded-xl border border-[rgba(216,214,211,.09)] bg-black/15 p-2.5"><div className="text-bone text-[12px]">{check.label}</div><Chip tone={check.status === 'pass' ? 'success' : check.status === 'blocker' ? 'danger' : 'warning'}>{statusLabel(check.status)}</Chip><div className="text-[11px] leading-relaxed text-[var(--bone-dim)]">{check.note}</div></div>)}</div></Panel>
          <Panel title="Релевантные ключи" icon={Layers3}><div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-2">{brief.candidateKeywords.length ? brief.candidateKeywords.map((kw, i) => <div key={`${kw.keyword_norm || kw.keyword}-${i}`} className="rounded-xl border border-[rgba(216,214,211,.10)] bg-black/15 p-2.5"><div className="text-bone text-[13px] leading-snug">{kw.keyword || kw.keyword_norm}</div><div className="mt-1.5 text-[10px] text-[var(--gold-warm)]">релевантность: {kw.pilot_relevance_score || 0}</div><div className="mt-1 text-[10px] leading-relaxed text-[var(--bone-dim)]">{kw.pilot_relevance_reason}</div></div>) : <div className="text-[12px] text-[var(--bone-dim)]">По сохранённому набору нет сильных candidate keywords. Вернись в Мастер листинга и смягчи фильтры или выбери другой режим.</div>}</div></Panel>
        </div>

        <Panel title="Предпросмотр SEO-черновика" icon={Sparkles}>
          <div className="grid lg:grid-cols-[.85fr_1fr] gap-5"><div className="space-y-3"><Preview label="SEO-заголовок" value={brief.draftPreview.seoTitle} /><Preview label="Главный заголовок H1" value={brief.draftPreview.h1} /><Preview label="Описание для Google" value={brief.draftPreview.metaDescription} /></div><div><div className="text-[10px] uppercase tracking-[0.18em] text-[var(--smoke)] mb-1.5">Черновой текст и тезисы</div><div className="rounded-xl border border-[rgba(216,214,211,.10)] bg-black/15 p-3 text-[12px] leading-relaxed text-[var(--bone-dim)]"><p>{brief.draftPreview.intro}</p><ul className="mt-3 space-y-1.5 list-disc pl-5">{brief.draftPreview.bullets.map((b) => <li key={b}>{b}</li>)}</ul></div></div></div>
...
function normalizeDecisionKeywords(value) { const rows = Array.isArray(value) ? value : []; return rows.map((row) => ({ ...row, keyword: row.keyword || row.keyword_norm, keyword_norm: row.keyword_norm || row.keyword, priority_tier: 'tier_1', validation_status: row.avg_monthly_searches ? 'validated' : 'queued', cleanup_pipeline_status: 'from_listing_master_decision', should_validate_api: false, should_hold: false })); }
