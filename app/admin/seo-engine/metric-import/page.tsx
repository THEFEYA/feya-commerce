...
function Pill({ children, danger = false }) {
  return <span className={`rounded-full border px-2 py-1 text-[10px] ${danger ? 'border-[rgba(196,64,88,.30)] text-[var(--ruby-soft)] bg-[rgba(160,32,56,.08)]' : 'border-[rgba(216,214,211,.10)] text-[var(--bone-dim)] bg-black/20'}`}>{children}</span>;
}

function ColumnCard({ column }) {
  return <div className="rounded-xl border border-[rgba(216,214,211,.10)] bg-black/15 p-3">
    <div className="flex items-start justify-between gap-2"><div className="text-bone text-[12px]">{column.label}</div>{column.required ? <Pill danger>обязательно</Pill> : <Pill>опционально</Pill>}</div>
    <div className="mt-1 text-[10px] text-[var(--smoke)]">{column.key}</div>
    <div className="mt-2 text-[10px] leading-relaxed text-[var(--bone-dim)]">{column.purpose}</div>
  </div>;
}

export default async function SeoMetricImportPage() {
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_80%_0%,rgba(212,178,106,.13),transparent_32%),linear-gradient(180deg,#07070A,#111016_45%,#07070A)]">
    <section className="container-feya pt-7 pb-12">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between border-b border-[rgba(216,214,211,.12)] pb-5 mb-5">
        <div>
          <div className="eyebrow-gold mb-2">Админка · SEO · metric import</div>
      ...
      <div className="grid lg:grid-cols-[1fr_340px] gap-5 mb-5">
        <Panel title="Назначение import contract" icon={DatabaseZap}>
        ...
        <Panel title="Что это даёт" icon={ShieldAlert}>
          <div className="space-y-2 text-[11px] leading-relaxed text-[var(--bone-dim)]">
            <div>1. Запрещает финальный scoring без источника, даты и региона.</div>
            <div>2. Разделяет Google Ads, CSV, eRank и DataForSEO, чтобы не смешивать разные данные вслепую.</div>
            <div>3. Даёт безопасный путь: сначала CSV/manual import, потом API adapter.</div>
          </div>
        </Panel>
      </div>

      <div className="grid lg:grid-cols-2 gap-5 mb-5">
        <Panel title="Источники метрик">
          <div className="grid sm:grid-cols-2 gap-3">{contract.sources.map((source) => <div key={source.id} className="rounded-xl border border-[rgba(216,214,211,.10)] bg-black/15 p-3"><div className="flex items-start justify-between gap-2"><div className="text-bone text-[13px]">{source.label}</div><Pill>{source.trust}</Pill></div><div className="mt-2 text-[10px] leading-relaxed text-[var(--bone-dim)]">{source.role}</div><div className="mt-2"><Pill danger={!source.allowedForScoring}>{source.allowedForScoring ? 'можно для scoring' : 'только support'}</Pill></div></div>)}</div>
        </Panel>
        <Panel title="Flow статусов">
          <div className="space-y-2">{contract.statusFlow.map((step) => <div key={step.status} className="rounded-xl border border-[rgba(216,214,211,.10)] bg-black/15 p-3"><div className="flex items-start justify-between gap-2"><div className="text-bone text-[12px]">{step.label}</div><Pill>{step.status}</Pill></div><div className="mt-1 text-[10px] text-[var(--bone-dim)]">{step.meaning}</div><div className="mt-1 text-[10px] text-[var(--smoke)]">Дальше: {step.nextStep}</div></div>)}</div>
        </Panel>
      </div>

      <Panel title="Колонки для импорта">
        <div className="mb-3 text-[11px] text-[var(--bone-dim)]">Обязательные поля должны быть в CSV или API response, иначе ключ не перейдёт к scoring.</div>
        <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-3">{[...contract.requiredColumns, ...contract.optionalColumns].map((column) => <ColumnCard key={column.key} column={column} />)}</div>
      </Panel>

      <div className="mt-5 grid lg:grid-cols-[.9fr_1.1fr] gap-5">
        <Panel title="Validation rules" icon={ShieldAlert}>
          <div className="space-y-2">{contract.validationRules.map((rule) => <div key={rule.label} className="rounded-xl border border-[rgba(216,214,211,.10)] bg-black/15 p-3"><div className="flex items-start justify-between gap-2"><div className="text-bone text-[12px]">{rule.label}</div><Pill danger={rule.severity === 'blocker'}>{rule.severity}</Pill></div><div className="mt-1 text-[10px] leading-relaxed text-[var(--bone-dim)]">{rule.rule}</div></div>)}</div>
        </Panel>
        <Panel title="CSV/API template preview">
          <pre className="max-h-[420px] overflow-auto whitespace-pre-wrap rounded-xl bg-black/30 border border-[rgba(216,214,211,.08)] p-3 text-[10px] leading-relaxed text-[var(--bone-dim)]">{JSON.stringify(contract.sampleRows, null, 2)}</pre>
        </Panel>
      </div>
    </section>
  </main>;
