...
export default async function MediaSeoPipelinePage() {
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_80%_0%,rgba(212,178,106,.13),transparent_32%),linear-gradient(180deg,#07070A,#111016_45%,#07070A)]"><section className="container-feya pt-10 pb-16">
    ...
    <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8"><Metric icon={ImageIcon} label="Images" value={summary.total || 0} note="Primary images in current storefront-candidate slice." /><Metric icon={RefreshCw} label="Needs Export" value={summary.exportRequired || 0} note="Needs controlled image export pass." tone="warning" /><Metric icon={Scaling} label="Resize" value={summary.resizeRecommended || 0} note="Resize or format recommended." tone="warning" /><Metric icon={CheckCircle2} label="Image Sitemap" value={summary.imageSitemapEligible || 0} note="Ready for image sitemap." tone="success" /><Metric icon={FileImage} label="Pinterest" value={summary.pinterestExportEligible || 0} note="Ready for Pinterest." tone="success" /></div>
