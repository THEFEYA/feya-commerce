...
import { mediaSeoNoteLabel, mediaSeoStageLabel } from '@/lib/adminDisplayRu';
...
export default async function MediaSeoDetailPage({ params }: PageProps) {
  ...
  if (error || !plan) {
    return <main className="min-h-screen bg-[#07070A]"><section className="container-feya pt-10 pb-16"><div className="rounded-2xl border border-[rgba(196,64,88,.35)] bg-[rgba(160,32,56,.10)] p-6 text-[var(--bone-dim)]">{error || 'План SEO медиа не найден.'}</div><Link href="/admin/media-seo" className="btn-ghost mt-5">Назад к SEO медиа</Link></section></main>;
  }
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_80%_0%,rgba(212,178,106,.13),transparent_32%),linear-gradient(180deg,#07070A,#111016_45%,#07070A)]"><section className="container-feya pt-10 pb-16">
    <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between border-b border-[rgba(216,214,211,.12)] pb-7 mb-7"><div><div className="eyebrow-gold mb-3">Админка · SEO медиа · Детали</div><h1 className="font-tall text-bone leading-none" style={{ fontSize: 'clamp(44px,7vw,88px)' }}>План изображения</h1><p className="mt-4 max-w-3xl text-[15px] leading-relaxed text-[var(--bone-dim)]">{plan.title}</p><div className="mt-4 flex flex-wrap gap-1.5"><Chip tone={toneForStage(plan.stage)}>{mediaSeoStageLabel(plan.stage)}</Chip>{plan.exportRequired ? <Chip tone="warning">Нужен экспорт</Chip> : null}{plan.resizeRecommended ? <Chip tone="warning">Нужно уменьшить</Chip> : null}{plan.imageSitemapEligible ? <Chip tone="success">Sitemap изображений</Chip> : null}{plan.pinterestExportEligible ? <Chip tone="success">Pinterest</Chip> : null}</div></div><div className="flex gap-3"><Link href="/admin/media-seo" className="btn-ghost">Media SEO <ArrowUpRight size={13} /></Link><Link href={`/admin/products/${plan.productSlug}`} className="btn-ghost">Карточка товара <ArrowUpRight size={13} /></Link></div></div>

    <div className="grid lg:grid-cols-[360px_1fr] gap-6 mb-8"><div className="rounded-2xl border border-[rgba(216,214,211,.12)] bg-black/20 p-4"><div className="relative aspect-[4/5] rounded-xl overflow-hidden bg-black/30 border border-[rgba(216,214,211,.10)]">{plan.imageUrl ? <img src={plan.imageUrl} alt="" className="absolute inset-0 h-full w-full object-cover" /> : <TriangleAlert size={24} className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-[var(--ruby-soft)]" />}</div></div><div className="grid sm:grid-cols-2 gap-4"><Metric icon={ImageIcon} label="Текущий файл" value={plan.currentFilename ? 'Есть' : 'Нет'} note={plan.currentFilename || 'Текущий файл изображения не найден.'} /><Metric icon={FileImage} label="Рекомендуемый файл" value="SEO" note={plan.suggestedFilename} tone="success" /><Metric icon={RefreshCw} label="Экспорт" value={plan.exportRequired ? 'Да' : 'Нет'} note="Нужен ли контролируемый экспорт изображения." tone={plan.exportRequired ? 'warning' : 'success'} /><Metric icon={Scaling} label="Размер / формат" value={plan.resizeRecommended ? 'Да' : 'Нет'} note="Рекомендация по размеру или формату изображения." tone={plan.resizeRecommended ? 'warning' : 'success'} /><Metric icon={CheckCircle2} label="Image sitemap" value={plan.imageSitemapEligible ? 'Готово' : 'Отложено'} note="Готовность для sitemap изображений." tone={plan.imageSitemapEligible ? 'success' : 'warning'} /><Metric icon={CheckCircle2} label="Pinterest" value={plan.pinterestExportEligible ? 'Готово' : 'Отложено'} note="Готовность для будущего процесса Pinterest." tone={plan.pinterestExportEligible ? 'success' : 'warning'} /></div></div>

    <div className="rounded-2xl border border-[rgba(216,214,211,.12)] bg-[rgba(255,255,255,.025)] p-5"><div className="eyebrow-gold mb-4">Что нужно сделать</div><div className="space-y-2">{plan.notes.length ? plan.notes.map((note, index) => <div key={index} className="rounded-xl border border-[rgba(216,214,211,.10)] bg-black/15 p-3 text-[13px] leading-relaxed text-[var(--bone-dim)]">{note}</div>) : <div className="rounded-xl border border-[rgba(108,183,138,.35)] bg-[rgba(108,183,138,.08)] p-3 text-[13px] leading-relaxed text-[#a9dfbd]">Открытых задач по SEO медиа нет.</div>}</div></div>
  </section></main>;
}
