...
const ADMIN_PRODUCT_BUILDER_DETAIL_VIEW = 'feya_commerce_v_step6_product_builder_detail';

const ADMIN_PRODUCT_BUILDER_DETAIL_SELECT = [
...
function normalizeBuilderConfigurations(row: any) {
  return asArray(row.configurations).map((configuration: any, index: number) => {
    ...
    return {
      ...
      needs_label_review: configuration.review_status && configuration.review_status !== 'approved',
    };
  });
}
...
function toBuilderProduct(row: any): StorefrontProduct {
...
async function getProduct(slug: string): Promise<{ product: StorefrontProduct | null; error?: string }> {
  ...
  return { product: builderData ? toBuilderProduct(builderData) : null };
}
