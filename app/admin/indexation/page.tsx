...
async function loadProducts() {
  ...
  const { data, error } = await supabase.from(STOREFRONT_VIEW_V4).select(STOREFRONT_V4_CARD_SELECT).limit(INDEXATION_PRODUCTS_LIMIT);
...
export default async function IndexationReadinessPage() {
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_80%_0%,rgba(212,178,106,.13),transparent_32%),linear-gradient(180deg,#07070A,#111016_45%,#07070A)]"><section className="container-feya pt-10 pb-16">
    ...
    <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8"><Metric icon={Rocket} label="Products" value={summary.total || 0} note="Current storefront-candidate v4 slice." /><Metric icon={ShieldAlert} label="Blocked" value={summary.Blocked || 0} note="Must not be opened to search." tone="danger" /><Metric icon={FileSearch} label="SEO approved" value={summary.seoApproved || 0} note="Approved through SEO Approval queue." tone="success" /><Metric icon={ImageIcon} label="Media ready" value={summary.mediaReady || 0} note="Image sitemap media-ready." tone="success" /><Metric icon={CheckCircle2} label="Preview-ready" value={summary['Ready for Preview'] || 0} note="Can move to noindex preview." tone="success" /></div>
    <div className="rounded-2xl border border-[rgba(216,214,211,.12)] bg-[rgba(255,255,255,.025)] overflow-hidden"><div className="grid grid-cols-[76px_1.15fr_.85fr_.75fr_.85fr_.85fr] gap-4 px-5 py-4 border-b border-[rgba(216,214,211,.10)] text-[10px] uppercase tracking-[0.22em] text-[var(--smoke)]"><div>Image</div><div>Product</div><div>Stage</div><div>SEO</div><div>Product</div><div>Media</div></div><div className="divide-y divide-[rgba(216,214,211,.08)]">{visiblePlans.map((plan) => <Link key={plan.productSlug} href={`/admin/seo-lab/${plan.productSlug}`} className="grid grid-cols-[76px_1.15fr_.85fr_.75fr_.85fr_.85fr] gap-4 items-center px-5 py-4 hover:bg-[rgba(212,178,106,.04)] transition-colors"><div className="relative h-20 w-16 rounded-lg overflow-hidden bg-black/30 border border-[rgba(216,214,211,.10)]">{plan.imageUrl ? <img src={plan.imageUrl} alt="" className="absolute inset-0 h-full w-full object-cover" /> : null}</div><div><div className="text-bone text-[15px] leading-snug line-clamp-2">{plan.title}</div><div className="mt-2 text-[10px] uppercase tracking-[0.18em] text-[var(--smoke)]">/{plan.productSlug}</div></div><div><Chip tone={toneForStage(plan.stage)}>{plan.stage}</Chip></div><div><Chip tone={plan.hasSeoApproval ? 'success' : 'warning'}>{plan.hasSeoApproval ? 'approved' : 'pending'}</Chip></div><div><Chip tone={plan.readiness.tone}>{plan.readiness.label}</Chip></div><div><Chip tone={plan.media.stage === 'Ready for Image Sitemap' ? 'success' : 'warning'}>{plan.media.stage}</Chip></div></Link>)}</div></div>
  </section></main>;
}
