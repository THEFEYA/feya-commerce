...
const STRATEGIES = ['balanced', 'demand', 'opportunity', 'niche', 'visual', 'commercial'];
...
const STRATEGY_LABELS = {
  ...
  niche: 'Нишевые long-tail',
  visual: 'Визуальный поиск',
  commercial: 'Коммерческий фокус',
};

const STRATEGY_NOTES = {
  balanced: 'Базовый режим: сначала совпадение с товаром, потом SEO-оценка, спрос и конкуренция.',
  demand: 'Поднимает слова с большим средним спросом. Полезно для сильных категорий и широких посадочных.',
  opportunity: 'Ищет шанс легче выйти выше: ненулевой спрос + низкая или средняя конкуренция.',
  niche: 'Поднимает более узкие слова под конкретный товар, чтобы не конкурировать со всеми сразу.',
  visual: 'Фокус на словах для изображений, ALT, Pinterest/Google Images и визуальных коллекций.',
  commercial: 'Фокус на словах с коммерческим намерением: outfit, costume, set, buy-ready формулировки.',
};
...
const PRESETS = [
  ...
  { label: 'Золотая броня Burning Man', type: 'product_or_alt', component: 'armor', material: 'gold', event: 'burning man' },
...
function strategyRank(row, strategy) {
  ...
  const commercialBoost = /outfit|costume|set|wear|clothing|for sale|buy/.test(keyword) || bucket.includes('commercial') ? 24 : 0;
  const visualBoost = bucket.includes('visual') || /mirror|gold|silver|holographic|reflective|metallic|image|photo/.test(keyword) ? 24 : 0;

  if (strategy === 'demand') return match * 1.15 + score + volumeBoost * 2;
  if (strategy === 'opportunity') return match * 1.35 + score + volumeBoost + competitionOpportunity + indexOpportunity;
  if (strategy === 'niche') return match * 1.7 + score + longTailBoost + competitionOpportunity - Math.max(0, volumeBoost - 30);
  if (strategy === 'visual') return match * 1.35 + score + visualBoost + volumeBoost + competitionOpportunity * 0.4;
  if (strategy === 'commercial') return match * 1.25 + score + commercialBoost + volumeBoost + competitionOpportunity * 0.5;
...
function strategySignal(row) {
  const volume = Number(row.avg_monthly_searches || 0);
  const competition = String(row.competition || '').toUpperCase();
  const type = String(row.bank_bucket || '');
  if (volume > 0 && competition === 'LOW') return { label: 'лёгкий шанс', tone: 'success' };
  if (type.includes('product') && volume <= 500 && volume > 0 && competition !== 'HIGH') return { label: 'нишевая точка', tone: 'success' };
  if (volume >= 1000 && competition === 'HIGH') return { label: 'большой спрос', tone: 'warning' };
  if (type.includes('visual')) return { label: 'визуальный слой', tone: 'gold' };
  if (type.includes('collection')) return { label: 'для посадочной', tone: 'gold' };
...
function productSeoStatus(product, filters, totalCount) {
  ...
  if (!totalCount) return { label: 'нет совпадений', tone: 'warning', note: 'сними один фильтр или поменяй тип использования слов' };
  return { label: 'готов к подбору', tone: 'success', note: 'есть одобренные слова под текущий SEO-угол товара' };
}
...
export default async function AdminListingMasterPage({ searchParams }) {
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_80%_0%,rgba(212,178,106,.13),transparent_32%),linear-gradient(180deg,#07070A,#111016_45%,#07070A)]">
    <section className="container-feya pt-10 pb-16">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6"><Metric icon={Database} label="Одобренных слов" value={keywordTypeCount(counts, 'all')} note="Только approved из SEO-ядра." tone="success" /><Metric icon={PackageSearch} label="Товары" value={formatNumber(visibleProducts)} note={`Показано из ${formatNumber(totalProducts)}. Картинки: ${imageSource}.`} tone="success" /><Metric icon={Layers3} label="Товар / ALT" value={keywordTypeCount(counts, 'product_or_alt')} note="Слова для карточки и картинок." tone="success" /><Metric icon={SearchCheck} label="Найдено сейчас" value={formatNumber(totalCount ?? rows.length)} note={dnaActive ? 'По текущему SEO-углу и стратегии.' : 'Без ДНК-фильтра.'} tone="warning" /></div>

      <div className="rounded-2xl border border-[rgba(212,178,106,.18)] bg-[rgba(212,178,106,.045)] p-5 mb-6"><div className="flex items-center gap-2 eyebrow-gold mb-2"><Sparkles size={14} /> Как читать этот экран</div><p className="text-[13px] leading-relaxed text-[var(--bone-dim)]">SEO-угол товара — это выбранный состав, материал/цвет и сценарий. Стратегия отбора — отдельная логика сортировки: больше спроса, ниже конкуренция, нишевые long-tail, визуальный или коммерческий фокус. В таблице: “Совпадение” = подходит ли слово товару, “SEO-оценка” = внутренний балл, “Средний спрос” = поиски Google в месяц, “Конкуренция” = Google Keyword Planner, “Тип использования” = куда лучше вставлять слово.</p></div>
