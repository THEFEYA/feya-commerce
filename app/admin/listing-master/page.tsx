...
async function saveDecisionAction(formData) {
  ...
  const payload = {
    ...
    decision_note: 'Saved from Listing Master v1 before SEO pack generation',
  };
...
async function loadProducts(productQuery = '') {
  ...
  const result = await supabase.from(FOCUS_VIEW).select(FOCUS_SELECT).limit(PRODUCT_LIMIT);
  if (result.error) return { products: [], totalProducts: 0, visibleProducts: 0, error: result.error.message, imageSource: 'focus_view_error' };

  const allProducts = (result.data || []).map((row) => normalizeProductFocus(row)).sort((a, b) => String(a.title || '').localeCompare(String(b.title || '')));
  ...
  return { products, totalProducts: allProducts.length, visibleProducts: products.length, error: null, imageSource: 'listing_master_focus_v1' };
}
...
function normalizeProductFocus(row) {
  ...
  const subtitle = [row.category_label || row.product_type || 'Product', row.world_label, row.canonical_color_label || row.color].filter(Boolean).join(' · ');
...
function applyDnaMatching(rows, filters) {
  ...
  const enriched = rows.map((row) => {
    ...
    const matchScore = active ? match.matchScore : null;
    return { ...row, match_score: matchScore, match_reasons: match.reasons.join(' · '), strategy_rank: strategyRank({ ...row, match_score: active ? match.matchScore : 0 }, filters.strategy) };
  });
...
function savedMessage(value) { if (value === 'ok') return { tone: 'success', text: 'Draft решения сохранён. Теперь это можно использовать для SEO-брифа и будущей аналитики.' }; if (value === 'error') return { tone: 'warning', text: 'Draft не сохранился. Проверь SUPABASE_SERVICE_ROLE_KEY или RLS/service route.' }; return null; }
...
function productSeoStatus(product, filters, totalCount) { if (!product) return { label: 'товар не выбран', tone: 'neutral', note: 'выбери товар слева или работай вручную через ДНК-фильтры' }; if (!filters.component && !filters.material && !filters.event) return { label: 'нужна ручная ДНК', tone: 'warning', note: 'авто-ДНК не распознала состав/материал/сценарий' }; if (!totalCount) return { label: 'нет совпадений', tone: 'warning', note: 'сними один фильтр или поменяй тип использования слов' }; return { label: 'готов к draft', tone: 'success', note: 'есть одобренные слова под текущий фокус товара' }; }
function buildHref(filters, patch = {}) { const next = { ...filters, ...patch }; const params = new URLSearchParams(); if (next.type && next.type !== 'all') params.set('type', next.type); if (next.strategy && next.strategy !== DEFAULT_STRATEGY) params.set('strategy', next.strategy); if (next.component) params.set('component', next.component); if (next.material) params.set('material', next.material); if (next.event) params.set('event', next.event); if (next.q) params.set('q', next.q); if (next.productId || next.product_id) params.set('product_id', next.productId || next.product_id); if (next.productQ || next.product_q) params.set('product_q', next.productQ || next.product_q); const query = params.toString(); return query ? `/admin/listing-master?${query}` : '/admin/listing-master'; }
...
export default async function AdminListingMasterPage({ searchParams }) {
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_80%_0%,rgba(212,178,106,.13),transparent_32%),linear-gradient(180deg,#07070A,#111016_45%,#07070A)]">
    <section className="container-feya pt-10 pb-16">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between border-b border-[rgba(216,214,211,.12)] pb-7 mb-7"><div><div className="eyebrow-gold mb-3">Админка · Listing Master · подбор SEO-слов</div><h1 className="font-tall text-bone leading-none" style={{ fontSize: 'clamp(44px,7vw,88px)' }}>Мастер листинга</h1><p className="mt-4 max-w-3xl text-[15px] leading-relaxed text-[var(--bone-dim)]">Сначала система предлагает фокус из реального product focus view. Ты можешь поправить состав, материал/цвет, событие и режим. Потом сохраняем draft решения, чтобы дальше сгенерировать SEO pack и сравнивать результат с будущей аналитикой.</p></div><div className="flex flex-wrap gap-3"><Link href="/admin/products" className="btn-ghost">Товары <ArrowUpRight size={13} /></Link><Link href="/admin/seo-keywords" className="btn-ghost">SEO-ядро <ArrowUpRight size={13} /></Link><Link href="/admin/seo-engine/briefs" className="btn-ghost">SEO-бриф <ArrowUpRight size={13} /></Link></div></div>
      ...
      <div className="rounded-2xl border border-[rgba(212,178,106,.18)] bg-[rgba(212,178,106,.045)] p-5 mb-6"><div className="flex items-center gap-2 eyebrow-gold mb-2"><Sparkles size={14} /> Как читать этот экран</div><p className="text-[13px] leading-relaxed text-[var(--bone-dim)]">Фокус товара берётся из safe view: title, product_type, material, color, world/category labels, изображения и component mapping. Ручная правка сохраняется в decision draft. Коммерческие слова вроде buy, price, order учитываются внутри ранжирования и через тип “Коммерческие посадочные”.</p></div>
      <div className="grid xl:grid-cols-[440px_1fr] gap-6 mb-6"><div className="rounded-2xl border border-[rgba(216,214,211,.12)] bg-[rgba(255,255,255,.025)] p-5 xl:sticky xl:top-6 self-start"><div className="flex items-center gap-2 eyebrow-gold mb-4"><PackageSearch size={14} /> Выбрать товар</div><form action="/admin/listing-master" className="mb-4 rounded-2xl border border-[rgba(216,214,211,.10)] bg-black/15 p-3"><div className="flex gap-2"><input name="product_q" defaultValue={rawFilters.productQ} placeholder="Поиск товара: gold, harness, Etsy ID, первые слова" className="w-full rounded-xl border border-[rgba(216,214,211,.14)] bg-black/25 px-3 py-2 text-[12px] text-bone outline-none focus:border-[rgba(212,178,106,.45)]" /><button type="submit" className="btn-ghost"><Search size={13} /> Найти</button></div><div className="mt-2 flex flex-wrap gap-2"><Link href="/admin/listing-master" className="text-[11px] text-[var(--gold-warm)] hover:underline">Сбросить поиск и выбор</Link><span className="text-[11px] text-[var(--bone-dim)]">Найдено: {formatNumber(visibleProducts)}</span></div></form><div className="max-h-[calc(100vh-330px)] min-h-[420px] space-y-2 overflow-auto pr-1">{products.slice(0, 140).map((product) => <ProductCard key={product.id} product={product} active={selectedProduct?.id === product.id} filters={filters} />)}{!products.length && !productsError ? <div className="text-[12px] text-[var(--bone-dim)]">По этому поиску товаров не найдено.</div> : null}</div></div>
        <div className="space-y-6"><div className="rounded-2xl border border-[rgba(216,214,211,.12)] bg-[rgba(255,255,255,.025)] p-5"><div className="flex items-center justify-between gap-3 mb-4"><div className="flex items-center gap-2 eyebrow-gold"><SlidersHorizontal size={14} /> Фокус товара</div><Chip tone={seoStatus.tone}>{seoStatus.label}</Chip></div>{selectedProduct ? <div className="grid sm:grid-cols-[96px_1fr] gap-4 rounded-2xl border border-[rgba(212,178,106,.18)] bg-[rgba(212,178,106,.055)] p-4 mb-4"><ProductImage product={selectedProduct} size="lg" /><div><div className="text-bone text-[18px] leading-snug">{selectedProduct.title}</div><div className="mt-1 text-[12px] text-[var(--bone-dim)]">{selectedProduct.subtitle}</div><div className="mt-3 flex flex-wrap gap-2"><Chip tone="success">состав: {labelFor('component', filters.component)}</Chip><Chip tone="gold">материал/цвет: {labelFor('material', filters.material)}</Chip><Chip tone="warning">сценарий: {labelFor('event', filters.event)}</Chip>{selectedProduct.hasComponentReviewRisk ? <Chip tone="warning">ДНК needs review: {selectedProduct.needsComponentReviewCount}</Chip> : null}</div><div className="mt-3 text-[11px] leading-relaxed text-[var(--bone-dim)]">Источник: {selectedProduct.categoryLabel || '—'} · {selectedProduct.worldLabel || '—'} · components: {selectedProduct.parentComponents.join(', ') || '—'} / {selectedProduct.childComponents.join(', ') || '—'}. {seoStatus.note}</div></div></div> : <div className="rounded-2xl border border-[rgba(216,214,211,.10)] bg-black/15 p-4 mb-4 text-[13px] leading-relaxed text-[var(--bone-dim)]">Товар ещё не выбран. Можно работать вручную через фильтры, но нормальный сценарий — найти товар слева и выбрать его.</div>}<div className="space-y-4"><div><div className="eyebrow-dim mb-2">Состав / часть товара</div><div className="flex flex-wrap gap-2">{COMPONENTS.map((item) => <FilterChip key={item} type="component" value={item} filters={filters} label={labelFor('component', item)} />)}</div></div><div><div className="eyebrow-dim mb-2">Материал / цвет / деталь</div><div className="flex flex-wrap gap-2">{MATERIALS.map((item) => <FilterChip key={item} type="material" value={item} filters={filters} label={labelFor('material', item)} />)}</div></div><div><div className="eyebrow-dim mb-2">Сценарий / событие</div><div className="flex flex-wrap gap-2">{EVENTS.map((item) => <FilterChip key={item} type="event" value={item} filters={filters} label={labelFor('event', item)} />)}</div></div></div></div>
          ...
          <div className="rounded-2xl border border-[rgba(216,214,211,.12)] bg-[rgba(255,255,255,.025)] p-5"><div className="eyebrow-dim mb-2">Поиск внутри одобренного SEO-ядра</div><form action="/admin/listing-master" className="rounded-2xl border border-[rgba(216,214,211,.10)] bg-black/15 p-4"><input type="hidden" name="type" value={filters.type} /><input type="hidden" name="strategy" value={filters.strategy} /><input type="hidden" name="component" value={filters.component} /><input type="hidden" name="material" value={filters.material} /><input type="hidden" name="event" value={filters.event} /><input type="hidden" name="product_id" value={filters.productId} /><input type="hidden" name="product_q" value={filters.productQ} /><input name="q" defaultValue={filters.q} placeholder="Например: gold rave, burning man, chest harness" className="w-full rounded-xl border border-[rgba(216,214,211,.14)] bg-black/25 px-4 py-3 text-[13px] text-bone outline-none focus:border-[rgba(212,178,106,.45)]" /><div className="mt-3 flex flex-wrap gap-2"><button className="btn-ghost" type="submit">Применить поиск слов</button><Link href="/admin/listing-master" className="btn-ghost">Очистить товар/ДНК</Link></div></form><div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{PRESETS.map((preset) => <PresetLink key={preset.label} preset={preset} filters={filters} />)}</div></div>
          <form action={saveDecisionAction} className="rounded-2xl border border-[rgba(108,183,138,.25)] bg-[rgba(108,183,138,.055)] p-5"><div className="eyebrow-gold mb-2">Зафиксировать решение</div><p className="text-[12px] leading-relaxed text-[var(--bone-dim)] mb-4">Сохранит выбранный фокус, режим и первые {KEYWORD_SNAPSHOT_LIMIT} предложенных ключей в decision draft. Это не публикует товар и не генерирует текст.</p><input type="hidden" name="canonical_product_id" value={selectedProduct?.id || ''} /><input type="hidden" name="product_slug" value={selectedProduct?.slug || ''} /><input type="hidden" name="matched_etsy_listing_id" value={selectedProduct?.etsyId || ''} /><input type="hidden" name="component" value={filters.component} /><input type="hidden" name="material" value={filters.material} /><input type="hidden" name="event" value={filters.event} /><input type="hidden" name="q" value={filters.q} /><input type="hidden" name="type" value={filters.type} /><input type="hidden" name="strategy" value={filters.strategy} /><input type="hidden" name="auto_focus_json" value={JSON.stringify(autoFocus)} /><input type="hidden" name="selected_keywords_json" value={JSON.stringify(selectedKeywords)} /><button className="btn-ghost" type="submit" disabled={!selectedProduct}><Save size={13} /> Сохранить draft решения</button></form>
        </div></div>
