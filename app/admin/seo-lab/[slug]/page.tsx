...
import { seoScoreNoteLabel } from '@/lib/adminDisplayRu';
...
function bucketLabel(bucket = '') {
  const labels = {
    ...
    long_tail_buyer_keywords: 'точный длинный',
...
export default async function SeoLabDetailPage({ params }: PageProps) {
  ...
  if (error || !score || !product) {
    return <main className="min-h-screen bg-[#07070A]"><section className="container-feya pt-10 pb-16"><div className="rounded-2xl border border-[rgba(196,64,88,.35)] bg-[rgba(160,32,56,.10)] p-6 text-[var(--bone-dim)]">{error || 'SEO-оценка не найдена.'}</div><Link href="/admin/seo-lab" className="btn-ghost mt-5">Назад в SEO-лабораторию</Link></section></main>;
  }
