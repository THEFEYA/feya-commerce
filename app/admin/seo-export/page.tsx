...
export default async function SeoExportQueuePage() {
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_80%_0%,rgba(212,178,106,.13),transparent_32%),linear-gradient(180deg,#07070A,#111016_45%,#07070A)]"><section className="container-feya pt-10 pb-16">
    <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between border-b border-[rgba(216,214,211,.12)] pb-7 mb-7"><div><div className="eyebrow-gold mb-3">Админка · SEO-экспорт</div><h1 className="font-tall text-bone leading-none" style={{ fontSize: 'clamp(44px,7vw,88px)' }}>SEO-экспорт</h1><p className="mt-4 max-w-3xl text-[15px] leading-relaxed text-[var(--bone-dim)]">Одобренные SEO-черновики для будущего применения и экспорта. Экран остаётся только для просмотра: sitemap, данные товаров, публичные SEO-поля и фиды здесь не изменяются.</p></div><div className="flex gap-3"><Link href="/admin/seo-approval" className="btn-ghost">Проверка SEO <ArrowUpRight size={13} /></Link><Link href="/admin/indexation" className="btn-ghost">Индексация <ArrowUpRight size={13} /></Link></div></div>
    ...
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8"><Metric icon={FileText} label="Одобренные черновики" value={approvedProducts.length} note="SEO-черновики, одобренные для очереди экспорта." tone="success" /><Metric icon={FileSearch} label="Показано" value={readyRows.length} note="Строки, показанные в очереди экспорта." /><Metric icon={ShieldAlert} label="Заблокировано" value={blockedRows.length} note="Одобрено, но всё ещё заблокировано базовой SEO-оценкой." tone={blockedRows.length ? 'danger' : 'neutral'} /><Metric icon={CheckCircle2} label="Автозапись" value="0" note="Автоматические записи в товар или SEO отключены." tone="success" /></div>
    <div className="space-y-4">{readyRows.map(({ score, draft }) => <article key={draft.productSlug} className="rounded-2xl border border-[rgba(216,214,211,.12)] bg-[rgba(255,255,255,.025)] p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><div className="text-bone text-[18px] leading-snug">{score.title}</div><div className="mt-2 text-[10px] uppercase tracking-[0.18em] text-[var(--smoke)]">/{draft.productSlug} · оценка {score.overallScore} · {score.stage}</div></div><div className="flex gap-2"><Link href={`/admin/seo-lab/${draft.productSlug}`} className="btn-ghost px-4 py-2 text-[10px]">Открыть оценку</Link><Link href={`/admin/products/${draft.productSlug}`} className="btn-ghost px-4 py-2 text-[10px]">Товар</Link></div></div><div className="mt-5 grid lg:grid-cols-2 gap-3"><DraftCell label="SEO-заголовок" value={draft.titleDraft} /><DraftCell label="Meta description" value={draft.metaDescriptionDraft} /><DraftCell label="H1" value={draft.h1Draft} /><DraftCell label="ALT изображения" value={draft.altTextDraft} /></div><div className="mt-4 rounded-xl border border-[rgba(212,178,106,.20)] bg-[rgba(212,178,106,.06)] p-3 text-[12px] leading-relaxed text-[var(--bone-dim)]">Подготовлено только для будущего экспорта или применения. Перед записью в публичные SEO-поля нужен отдельный механизм применения и журнал изменений.</div></article>)}</div>
  </section></main>;
}
