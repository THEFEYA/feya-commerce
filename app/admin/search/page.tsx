  ...
  return (
    <main className="owner-page">
      <div className="owner-page-inner">
        ...
        <form action="/admin/search" method="get" className="owner-card" style={{ marginBottom: '18px' }}>
        ...
        {workVM.length ? <section className="owner-section"><div className="owner-section-head"><h2>Работа</h2></div><div className="owner-list">
          {workVM.map((item) => <Link className="owner-list-row" href="/admin/work" key={item.id}><div className="owner-list-row-main"><h3>{item.title}</h3><p>{item.statusLabel} · {item.ownerLabel}</p></div></Link>)}
        </div></section> : null}
        ...
        {signalVM.length ? <section className="owner-section"><div className="owner-section-head"><h2>Сигналы</h2></div><div className="owner-list">
          {signalVM.map((item) => <Link className="owner-list-row" href="/admin/signals" key={item.id}><div className="owner-list-row-main"><h3>{item.title}</h3><p>{item.summary}</p></div></Link>)}
        </div></section> : null}
      </div>
    </main>
  );
