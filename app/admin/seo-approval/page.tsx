...
export default async function SeoApprovalPage() {
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_80%_0%,rgba(212,178,106,.13),transparent_32%),linear-gradient(180deg,#07070A,#111016_45%,#07070A)]"><section className="container-feya pt-10 pb-16">
    ...
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8"><Metric icon={Database} label="Сохранённые" value={savedDrafts.length} note="Новые SEO-pack drafts из storage layer." tone="success" /><Metric icon={ShieldAlert} label="На проверке" value={notReviewed} note="Ждут human review." tone="danger" /><Metric icon={CheckCircle2} label="Validator OK" value={validatorReady} note="Черновики проходят output validator." tone="success" /><Metric icon={Clock3} label="Similarity" value={needsSimilarity} note="Нужна отдельная проверка похожести/каннибализации." /></div>
    ...
    <div className="mb-10">
      ...
      {savedDrafts.length ? <div className="space-y-4">{savedDrafts.map((draft) => <SavedDraftCard key={draft.id} draft={draft} />)}</div> : <div className="rounded-2xl border border-[rgba(216,214,211,.12)] bg-[rgba(255,255,255,.025)] p-5 text-[var(--bone-dim)]">Сохранённых SEO-черновиков пока нет. Открой проверку черновика и нажми “Сохранить черновик для проверки”.</div>}
    </div>
