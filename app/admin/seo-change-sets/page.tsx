...
export default async function SeoChangeSetsPage() {
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_80%_0%,rgba(212,178,106,.13),transparent_32%),linear-gradient(180deg,#07070A,#111016_45%,#07070A)]"><section className="container-feya pt-10 pb-16">
    ...
    <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-8"><Metric icon={Database} label="Rows" value={rows.length} note="Stored rows." /><Metric icon={FileText} label="Pending" value={pending.length} note="Needs review." /><Metric icon={FileText} label="Approved" value={approved.length} note="Approved rows." /></div>
    <div className="rounded-2xl border border-[rgba(216,214,211,.12)] bg-[rgba(255,255,255,.025)] overflow-hidden"><div className="grid grid-cols-[1fr_.75fr_.85fr_1fr_1fr] gap-4 px-5 py-4 border-b border-[rgba(216,214,211,.10)] text-[10px] uppercase tracking-[0.22em] text-[var(--smoke)]"><div>Product</div><div>Status</div><div>Field</div><div>Current</div><div>Proposed</div></div><div className="divide-y divide-[rgba(216,214,211,.08)]">{rows.length ? rows.map((row) => <Link key={row.change_set_id} href={`/admin/products/${row.product_slug}`} className="grid grid-cols-[1fr_.75fr_.85fr_1fr_1fr] gap-4 px-5 py-4 hover:bg-[rgba(212,178,106,.04)] transition-colors"><div className="text-bone text-[14px] leading-snug">/{row.product_slug}</div><div><Chip>{row.status}</Chip></div><div className="text-[12px] leading-relaxed text-[var(--bone-dim)]">{row.target_field}</div><div className="text-[12px] leading-relaxed text-[var(--bone-dim)] line-clamp-3">{row.current_value || '—'}</div><div className="text-[12px] leading-relaxed text-bone line-clamp-3">{row.proposed_value || '—'}</div></Link>) : <div className="p-6 text-[13px] leading-relaxed text-[var(--bone-dim)]">No stored change sets yet.</div>}</div></div>
  </section></main>;
}
