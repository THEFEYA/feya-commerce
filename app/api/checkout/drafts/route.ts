...
export async function POST(request: Request) {
  ...
  const draftInsert = {
    ...
    price_contract_version: 'frontend_temp_v1',
    has_price_review_warning: true,
    has_label_review_warning: false,
  ...
  const draftItems = items.map((item) => {
    ...
    const unit = asAmount(item.price);
    ...
    return {
      ...
      configuration_id: asText(item.id) || null,
      configuration_label: asText(item.config) || 'Option',
      component_code: null,
      component_family: null,
      is_full_set: /full\s*set|complete\s*set/i.test(asText(item.config)),
      ...
      price_confidence_status: 'frontend_temp_v1',
      label_confidence_status: 'frontend_temp_v1',
