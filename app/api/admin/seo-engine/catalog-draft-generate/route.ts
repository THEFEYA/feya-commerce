...
function buildFinalReviewPrompt(
  ...
  const finalUser = [
    ...
    'About contains the exact Primary once, explains the whole product in one selected setting, and adds a different buyer value. Use two short sentences: the first connects the exact Primary to the selected occasion and buyer job; the second may name one observed component only when it explains a concrete result. Because separately selectable components form this set, never describe them as one piece, one continuous or unbroken line, a line from one body area to another, or a single design running between components. Do not repeat the intro, and do not repeat straps, adjustment, fit or comfort from the fixed panel.',
    ...
    'Read About, Why and Ideal aloud once. Replace “warrior line”, “armored effect”, “visually open”, “continuous line”, “cohesive styling” and similar design-review shorthand with a plain statement of what the wearer can combine, do or photograph.',
