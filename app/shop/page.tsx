...
async function getProducts() {
  ...
  const v3 = await supabase.from(STOREFRONT_VIEW_V3).select(STOREFRONT_CARD_SELECT).limit(120);
  ...
  if (!v2.error && v2.data?.length) return { products: v2.data, error: v3.error ? `v3 fallback active: ${v3.error.message}` : undefined };
