...
const bindings = () => products.flatMap((p:any)=>p.desired_bindings as any[]);
...
test('repair plan preserves known commercial values and manual overrides',()=>{
  ...
  for(const x of rows.filter((x:any)=>x.manual_override_amount==null&&x.source_amount!=null))assert.equal(x.public_price_amount,x.source_amount);
});
...
test('collapsed source options are separated into exact target configuration identities',()=>{
  ...
  assert.deepEqual(p057.desired_rows.map((x:any)=>x.normalized_key),['male_outfit','female_outfit','full_set_couple_owner_20260917']);
  assert.deepEqual(p560.desired_rows.map((x:any)=>x.normalized_key),['skirt','top_shoulders','full_set']);
  assert.equal(new Set(p057.desired_rows.map((x:any)=>x.sellable_configuration_id)).size,3);
  assert.equal(new Set(p560.desired_rows.map((x:any)=>x.sellable_configuration_id)).size,3);
});
