...
const products:any[] = plan.manual_lane.products as any[];
const allBindings = ():any[] => products.flatMap((p:any)=>p.desired_bindings as any[]);
...
test('manual lane accounts for exactly six immutable price identities and three proposed new configurations',()=>{
  ...
  assert.equal(new Set(rows.map((x:any)=>x.configuration_price_id)).size,6);
  assert.equal(rows.filter((x:any)=>x.config_operation==='create_and_rebind').length,3);
...
test('repair plan preserves known commercial values and manual overrides',()=>{
  const rows=allBindings();
  const byId=new Map<string,any>(rows.map((x:any)=>[x.configuration_price_id,x]));
  ...
  for(const x of rows.filter((x:any)=>x.manual_override_amount==null&&x.source_amount!=null)){
    assert.equal(x.public_price_amount,x.source_amount);
  }
});

test('collapsed source options are separated into exact target configuration identities',()=>{
  const p057:any=products.find((p:any)=>p.canonical_product_id.startsWith('057fbd51'));
  const p560:any=products.find((p:any)=>p.canonical_product_id.startsWith('5602d557'));
  assert.ok(p057);
  assert.ok(p560);
  assert.deepEqual(p057.desired_bindings.map((x:any)=>x.normalized_key),['male_outfit','female_outfit','full_set_couple_owner_20260917']);
  assert.deepEqual(p560.desired_bindings.map((x:any)=>x.normalized_key),['skirt','top_shoulders','full_set']);
  assert.equal(new Set(p057.desired_bindings.map((x:any)=>x.sellable_configuration_id)).size,3);
  assert.equal(new Set(p560.desired_bindings.map((x:any)=>x.sellable_configuration_id)).size,3);
});
