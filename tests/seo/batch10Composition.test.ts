...
const products=JSON.parse(fs.readFileSync(new URL('./fixtures/batch10Offers.json',import.meta.url),'utf8'));
...
test('Batch10 grouped choices keep separate identities, contents and original prices',()=>{
 for(const p of [products[1],products[3]]){
  ...
  const signature=JSON.parse(truth.signature.split('storefront-sellable-offer-v1:')[1]);
...
test('Batch10 exact reviewed outfit intent preserves metrics and factual gates',()=>{
 ...
 const run=(f=focus,k=keyword,prod=product)=>recommendCatalogKeywords({product:prod,focus:f,approvedKeywords:[k]}).keywords;
