...
test('blocks negative comparison, system language, selector narration and generic movement filler', () => {
  ...
  const result = validateSeoCommercialCopy(value, { editorial_policy_version: 'brand_mission_v2' });
  const codes = result.issues.map((issue) => issue.code);
...
test('historical read validation retains its policy; editing enforces the current policy without mutating evidence', () => {
  const value = draft({ intro: 'Our modular system adds movement rather than a fixed look. Choose one option.' });
  const original = JSON.stringify(value);
  const legacy = validateSeoCommercialCopy(value);
  assert.equal(legacy.editorial_policy_version, 'legacy_v1');
  assert.equal(legacy.issues.some(issue => issue.code === 'customer_copy_uses_robotic_system_language'), false);
  const edited = stampCurrentSeoEditorialPolicy(value);
  const result = validateSeoCommercialCopy(edited, { editorial_policy_version: 'legacy_v1' });
  assert.equal(result.editorial_policy_version, 'brand_mission_v2');
  assert.ok(result.issues.some(issue => issue.code === 'customer_copy_uses_robotic_system_language'));
  assert.equal(JSON.stringify(value), original);
});

test('unknown editorial policy is blocked and cannot be treated as an old approval', () => {
  assert.ok(validateSeoCommercialCopy(draft({ editorial_policy_version: 'unrecognized' })).issues.some(issue => issue.code === 'editorial_policy_version_invalid'));
});

test('current policy survives save normalization and permits a factual-detail-free brand close', () => {
  const body = 'At TheFEYA, we create original fashion for people who want to express their individuality. Our designs begin with imagination and a belief in the freedom to be yourself. We want our work to help you share your personality, celebrate your own ideas and make personal expression part of getting dressed.';
  const value = stampCurrentSeoEditorialPolicy(draft({ pdp_blocks: draft().pdp_blocks.map(block => block.block_key === 'main_description' ? { ...block, body } : block) }));
  const normalized = normalizeReviewDraftForSeoPack(value, {});
  assert.equal(normalized.editorial_policy_version, 'brand_mission_v2');
  assert.equal(normalized.pdp_blocks.find(block => block.block_key === 'main_description')?.body, body);
  const closingIssues = validateSeoCommercialCopy(normalized).issues.filter(issue => issue.code.startsWith('self_expression_close'));
