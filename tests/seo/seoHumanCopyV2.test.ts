import { SEO_EDITORIAL_MEMORY_V7 } from '../../lib/seoEditorialMemory.ts';
...
test('writer brief uses current offer and excludes raw legacy wording', () => {
  ...
  assert.equal(preflight.ok, true, JSON.stringify(preflight.issues));
  assert.equal(brief.editorial_reference, 'seo_editorial_memory_v7');
...
test('OpenAI writer performs one bounded request and reports token usage', async () => {
  ...
  try {
    ...
    assert.equal(result.output?.editorial_policy_version, 'brand_mission_v2');
...
test('current compact system and user memory share the versioned brand-mission policy', () => {
  const { brief, prompt } = buildCompactSeoWriterPrompt(inputContract());
  assert.equal(brief.editorial_reference, 'seo_editorial_memory_v7');
  const data = JSON.parse(prompt.user_prompt.split('\n').at(-1)!);
  assert.equal(data.editorial_reference, SEO_EDITORIAL_MEMORY_V7.contract_version);
  const close = SEO_EDITORIAL_MEMORY_V7.positive_block_frames.studio_close;
  assert.ok(prompt.system_prompt.includes(close));
  assert.match(close, /individuality, creative freedom/);
  assert.doesNotMatch(close, /Use body_identity_variant_en/);
});
