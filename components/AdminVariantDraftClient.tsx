  ...
  return <div id="product-variants" data-testid="variant-editor" className="space-y-4 text-[13px] text-[var(--bone-dim)]">
    ...
    {context && draft ? <>
      ...
      <fieldset disabled={locked} className="space-y-3"><legend className="eyebrow-gold mb-2">Комплектации</legend>
        ...
        <div className="grid gap-2 sm:grid-cols-[1fr_auto]"><label>Исходная комплектация<select aria-label="Исходная комплектация" className={field} value={newConfiguration} onChange={e=>setNewConfiguration(e.target.value)}>
      ...
      {(['colors','sizes'] as const).map(dimension => {
        ...
        return <fieldset key={dimension} disabled={locked} className="space-y-2"><legend className="eyebrow-gold mb-2">{isColor?'Цвета':'Размеры'}</legend>
          {draft[dimension].map((a,index)=><div key={a.id} data-attribute-id={a.id} className="grid grid-cols-[1fr_140px] gap-2">
            ...
            <label>Статус: {caption.toLowerCase()} {index+1}<select aria-label={`Статус: ${caption.toLowerCase()} ${index+1}`} className={field} value={a.state} onChange={e=>edit(s=>updateEditorAttribute(s,dimension,a.id,{state:e.target.value as VariantAttribute['state']}))}>{statuses.map(([state,label])=><option key={state} value={state}>{label}</option>)}</select></label>
          </div>)}
      ...
      <fieldset disabled={locked} className="space-y-3"><legend className="eyebrow-gold mb-2">Сочетания</legend>
        ...
        {draft.variants.map(v=><div key={v.variant_id} data-variant-id={v.variant_id} className="rounded-xl border border-[rgba(216,214,211,.10)] bg-black/15 p-3">
          ...
          <label>Статус сочетания<select aria-label="Статус сочетания" className={field} value={v.state} onChange={e=>edit(s=>({...s,variants:s.variants.map(row=>row.variant_id===v.variant_id?{...row,state:e.target.value as 'draft'|'retired'}:row)}))}><option value="draft">Черновик</option><option value="retired">Архив</option></select></label>
        </div>)}
        <div className="grid gap-2 sm:grid-cols-3">
          <label>Комплектация сочетания<select aria-label="Комплектация сочетания" className={field} value={tupleConfiguration} onChange={e=>setTupleConfiguration(e.target.value)}><option value="">Выберите</option>{draft.configurations.map(c=><option key={c.configuration_price_id} value={c.configuration_price_id}>{configName(c.configuration_price_id)}</option>)}</select></label>
          <label>Цвет сочетания<select aria-label="Цвет сочетания" className={field} value={tupleColor} onChange={e=>setTupleColor(e.target.value)}><option value="">Выберите</option><option value="none">Не применяется</option>{draft.colors.filter(a=>a.state!=='retired').map(a=><option key={a.id} value={a.id}>{a.label}</option>)}</select></label>
          <label>Размер сочетания<select aria-label="Размер сочетания" className={field} value={tupleSize} onChange={e=>setTupleSize(e.target.value)}><option value="">Выберите</option><option value="none">Не применяется</option>{draft.sizes.filter(a=>a.state!=='retired').map(a=><option key={a.id} value={a.id}>{a.label}</option>)}</select></label>
        </div>
