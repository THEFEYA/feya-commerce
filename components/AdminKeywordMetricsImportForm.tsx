...
export function AdminKeywordMetricsImportForm() {
  ...
  async function submitImport() {
    ...
    try {
      ...
      setIsLoading(false);
      setError('JSON не читается. Проверь кавычки, запятые и формат массива rows.');
      return;
    }
  ...
  return <div className="rounded-2xl border border-[rgba(216,214,211,.12)] bg-[rgba(255,255,255,.025)] p-5">
    <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between mb-5">
      <div>
        ...
        <div className="text-[13px] leading-relaxed text-[var(--bone-dim)]">Вставь JSON payload после CSV-конвертации. На следующем шаге добавим CSV parser, но API уже принимает нормализованные rows.</div>
      </div>
    ...
    <div className="mt-4 flex flex-wrap items-center gap-3">
      ...
      <button type="button" onClick={() => setPayload(JSON.stringify(SAMPLE_PAYLOAD, null, 2))} className="btn-ghost">Вернуть пример</button>
    </div>
