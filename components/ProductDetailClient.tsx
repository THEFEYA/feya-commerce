...
import { ArrowUpRight, ChevronLeft, ChevronRight, Heart, Share2, Truck, RotateCcw, FileText, ShoppingBag, Check, Zap, Ruler } from 'lucide-react';
  ...
  return <div data-testid="product-page" className="relative pt-24 lg:pt-28">
    ...
    <section className="container-feya pb-12 grid grid-cols-12 gap-6 lg:gap-12">
      ...
      <aside className="col-span-12 lg:col-span-5 lg:sticky lg:top-28 self-start">
        ...
        <div className="mt-7"><div className="flex items-center justify-between mb-2"><div className="eyebrow text-[10.5px]">Color · {colors[colorIdx] || colors[0] || 'Mirror'}</div><div className="eyebrow-dim">{colors.length || 1} shade</div></div><div className="flex gap-2">{colors.map((c,i)=><button key={c+i} onClick={()=>setColorIdx(i)} className={`w-10 h-10 rounded-full border-2 ${i===colorIdx?'border-white':'border-[rgba(216,214,211,0.28)]'}`} style={colorStyle(c)} title={c} />)}</div></div>
        <div className="mt-7"><div className="flex items-center justify-between mb-2"><div className="eyebrow text-[10.5px]">Size · {size}</div><a href="#description" className="eyebrow-dim hover:text-white flex items-center gap-1"><Ruler size={12} /> Size guide</a></div><div className="flex flex-wrap gap-2">{SIZES.map(s=><button key={s} onClick={()=>setSize(s)} className={`size-pill ${size===s && s !== 'Custom' ? 'size-pill-active' : ''} ${size===s && s === 'Custom' ? 'size-pill-custom' : ''}`}>{s}</button>)}</div></div>
        <div className="mt-7"><div className="eyebrow text-[10.5px] mb-3">Delivery</div><div className="grid grid-cols-2 gap-3"><button onClick={()=>setDelivery('standard')} className={`glass rounded-md px-4 py-4 text-left ${delivery==='standard'?'border-white':''}`}><Truck size={16} /><div className="font-semibold mt-2">Standard UPS</div><div className="text-xs text-[var(--bone-dim)]">14–21 business days · Included</div></button><button onClick={()=>setDelivery('express')} className={`glass rounded-md px-4 py-4 text-left ${delivery==='express'?'border-white':''}`}><Zap size={16} className="text-[var(--gold)]" /><div className="font-semibold mt-2">Express DHL</div><div className="text-xs text-[var(--bone-dim)]">7–10 business days · +$45</div></button></div><p className="text-[13px] text-[var(--bone-dim)] mt-3">Production time is calculated before shipping. Made to order.</p></div>
        <div className="mt-7 glass rounded-md p-4 grid grid-cols-3 gap-4"><div><div className="eyebrow-dim">Ordered</div><strong>Jun 12</strong></div><div><div className="eyebrow-dim">Ready</div><strong>Jun 26 – Jul 03</strong></div><div><div className="eyebrow-dim">Delivered</div><strong className="text-[var(--gold-warm)]">{delivery==='express'?'Jul 03 – Jul 10':'Jul 10 – Jul 24'}</strong></div></div>
        <div className="mt-7 border-t border-[rgba(216,214,211,0.12)] pt-5 flex items-end justify-between"><div className="eyebrow">Total · {qty} × {formatPrice(sale, currency)}</div><div className="font-price text-bone text-[40px]">{formatPrice(total, currency)}</div></div>
