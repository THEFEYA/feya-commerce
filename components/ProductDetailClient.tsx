...
function DisplayBody({ body }: { body: string }) {
  ...
  return <ul className="m-0 list-none space-y-2 p-0">{lines.map((line, index) => <li key={`${line}-${index}`} className="flex gap-2"><span className="mt-[.7em] h-1 w-1 shrink-0 rounded-full bg-[var(--gold-warm)]" /><span>{line}</span></li>)}</ul>;
}
