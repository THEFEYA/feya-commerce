...
function fallbackMedia(product: StorefrontProduct): StorefrontMedia[] {
...
function sortedOptions(product: StorefrontProduct) {
  return asConfigurations(product)
    .slice()
    .sort((a, b) => {
...
function optionKey(option: StorefrontConfiguration, index: number) {
...
function getDefaultOption(options: StorefrontConfiguration[]) {
  return options.find((option, index) => /full\s*set|complete/i.test(optionLabel(option, index))) || options[0] || null;
}

function storeBag(product: StorefrontProduct, qty: number, option: StorefrontConfiguration | null, size: string, delivery: string) {
  const key = 'feya_visual_bag_items';
  const current = JSON.parse(globalThis.localStorage?.getItem(key) || '[]') as unknown[];
  current.push({
    id: `${product.canonical_product_id}-${Date.now()}`,
    product_id: product.canonical_product_id,
    title: product.card_title || product.h1,
    qty,
    option: option ? optionLabel(option) : null,
    size,
    delivery,
  });
  globalThis.localStorage?.setItem(key, JSON.stringify(current));
  globalThis.localStorage?.setItem('feya_visual_bag', String(current.length));
  globalThis.dispatchEvent(new CustomEvent('feya-bag-updated', { detail: current.length }));
}
...
export function ProductDetailClient({ product, related }: ProductDetailClientProps) {
  ...
  const currency = (selectedOption?.currency || product.currency || 'EUR') as string;
    ...
    <section className="feya-pdp">
      ...
      <div className="feya-pdp-top">
        <div className="feya-gallery">
          <div className="feya-thumbs">
            {media.map((item, index) => (
              <button className={`feya-thumb ${selectedMedia === index ? 'active' : ''}`} key={`${item.url}-${index}`} type="button" onClick={() => setSelectedMedia(index)}>
                <Image src={String(item.url)} alt={String(item.alt || title)} fill sizes="120px" />
              </button>
            ))}
          </div>
          <div className="feya-main-photo">
            {selectedImage ? <Image src={String(selectedImage)} alt={product.primary_image_alt || title} fill priority sizes="(max-width: 900px) 92vw, 52vw" /> : <div className="feya-missing-image">Missing image</div>}
            <span className="feya-gallery-count">{selectedMedia + 1} / {media.length || 1}</span>
          </div>
        </div>
        ...
        <aside className="feya-buybox">
          ...
          <div className="feya-pdp-price">
            ...
            {product.min_price && product.max_price ? <span className="feya-pdp-range">Range {money(product.min_price, product.currency || currency)} – {money(product.max_price, product.currency || currency)}</span> : null}
          </div>
          ...
          <div className="feya-form-block">
            ...
            <select className="feya-select" value={selectedKey} onChange={(event) => setSelectedKey(event.target.value)}>
              {options.map((option, index) => <option key={optionKey(option, index)} value={optionKey(option, index)}>{optionLabel(option, index)}</option>)}
            </select>
          </div>
          ...
          <div className="feya-form-block">
            <div className="feya-label-row"><span>Color · {colorLabel(product)}</span><span>{colors.length} shade{colors.length > 1 ? 's' : ''}</span></div>
            <div className="feya-color-row">
              {colors.map((color) => <button key={color} type="button" className="feya-filter-swatch"><i className={`feya-swatch-${color.toLowerCase().replace(/[^a-z]+/g, '')}`} /><span>{color}</span></button>)}
            </div>
          </div>

          <div className="feya-form-block">
            <div className="feya-label-row"><span>Size · {size}</span><a href="#sizing">Size guide</a></div>
            <div className="feya-size-row">{['XS','S','M','L','XL','XXL','XXXL','Custom'].map((item) => <button type="button" className={`feya-size-pill ${item === size ? 'active' : ''}`} key={item} onClick={() => setSize(item)}>{item}</button>)}</div>
          </div>

          <div className="feya-form-block">
            <div className="feya-label-row"><span>Delivery</span></div>
            <div className="feya-delivery-row">
              <button type="button" className={`feya-delivery-card ${delivery === 'standard' ? 'active' : ''}`} onClick={() => setDelivery('standard')}><strong>Standard UPS</strong><span>14–21 business days · Included</span></button>
      ...
      <section className="feya-description">
        ...
        <div className="feya-detail-list">
          ...
          <div id="sizing"><h3>Sizing & fit</h3><p>Available in XS–XXXL standard sizing with custom sizing option.</p></div>
          <div id="shipping"><h3>Shipping & delivery</h3><p>Standard UPS 14–21 business days or Express DHL 7–10 business days after production.</p></div>
          <div id="returns"><h3>Returns & exchanges</h3><p>Standard-size pieces follow store policy. Custom-sized pieces are final sale after production begins.</p></div>
