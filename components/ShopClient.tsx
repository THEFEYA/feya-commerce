...
function includesText(product: StorefrontProduct, query: string) {
...
function textHas(product: StorefrontProduct, value: string) {
...
export function ShopClient({ products, error }: ShopClientProps) {
  ...
  return (
    <>
      <nav className="feya-category-bar" aria-label="Product categories">
        {categories.map((item) => (
          <button key={item} className={`feya-category-link ${category === item ? 'active' : ''}`} type="button" onClick={() => setCategory(item)}>
            {item}
          </button>
        ))}
        <label className="feya-sort feya-sort-select">
          Sort ·
      ...
      <section className="feya-shop-layout">
        <aside className="feya-filter-panel">
          ...
          <div className="feya-filter-group">
            <h4>Search</h4>
            <input className="feya-search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search atelier…" />
          </div>
          <div className="feya-filter-group">
            <h4>Price</h4>
            <div className="feya-price-range">
              <div className="feya-filter-values"><span>€{minPrice}</span><span>€{maxPrice}+</span></div>
              <input className="feya-range-input" type="range" min="0" max="1000" step="10" value={minPrice} onChange={(event) => setMinPrice(Math.min(Number(event.target.value), maxPrice - 10))} />
              <input className="feya-range-input" type="range" min="0" max="1000" step="10" value={maxPrice} onChange={(event) => setMaxPrice(Math.max(Number(event.target.value), minPrice + 10))} />
              <div className="feya-filter-values"><input value={minPrice} onChange={(event) => setMinPrice(Number(event.target.value) || 0)} /><span>to</span><input value={maxPrice} onChange={(event) => setMaxPrice(Number(event.target.value) || 1000)} /></div>
            </div>
          </div>
          <div className="feya-filter-group">
            <h4>Color</h4>
            <div className="feya-filter-swatches">
              {colors.map((color) => (
                <button type="button" className={`feya-filter-swatch ${selectedColor === color ? 'active' : ''}`} key={color} onClick={() => setSelectedColor(selectedColor === color ? null : color)}>
                  <i className={`feya-swatch-${color.toLowerCase()}`} />
                  <span>{color}</span>
                </button>
              ))}
            </div>
          ...
          <div className="feya-filter-group">
            <h4>Size</h4>
            <div className="feya-size-grid">
              {sizes.map((size) => <button type="button" className={selectedSize === size ? 'active' : ''} key={size} onClick={() => setSelectedSize(selectedSize === size ? null : size)}>{size}</button>)}
            </div>
          </div>
        ...
        <div>
          <div className="feya-shop-main-head">
            <span>{filtered.length} of {products.length} pieces</span>
            <span className="feya-sort">{sort}</span>
          </div>
          ...
          <section className="feya-grid">
            {filtered.map((product, index) => <ProductCard key={product.canonical_product_id} product={product} priority={index < 4} />)}
          </section>
        </div>
      </section>
    </>
