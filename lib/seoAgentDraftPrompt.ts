...
function buildSystemPrompt(input: SeoAgentInputContract) {
  ...
  return [
    ...
    'Generate only the product-specific LEFT PDP description. The live storefront owns the right information panel and the admin preview must reuse the same storefront component.',
    ...
    'main_description must connect the product to self-expression, individuality, visual identity, and the studio-created concept. Mention customization only briefly because operational details live in the right panel.',
    ...
    'Do not generate whats_included, sizing_fit, production_timing, shipping_delivery, material, care, customization, returns_exchanges, handmade_variation, or any right_info_panel block.',
    ...visionRules,
...
function buildUserPrompt(input: SeoAgentInputContract) {
  ...
  return [
    ...
    '- intro: 2-4 sentences. Lead with product benefit, silhouette, event or use case, and supported facts. The brand name may appear at most once across intro plus all left blocks combined.',
    ...
    '- about_this_piece / left_description: product-first opening story. Include exact supported material, finish, feel, structure, or silhouette naturally when useful.',
    '- why_youll_love_it / left_description: 3-5 non-duplicative benefits from at least three distinct categories. Only one design or handmade uniqueness point.',
    '- ideal_for / left_description: grounded event, audience, and use-case bullets.',
    '- main_description / left_description: heading must be Designed for self-expression. Finish with a concise conversion paragraph about individuality, visual identity, studio-created design, and brief supported customization.',
    '',
...
export function promptGuardrails(input?: SeoAgentInputContract) {
  return [
    ...
    'What’s included and all right-panel blocks are storefront-controlled and must not be generated by OpenAI.',
