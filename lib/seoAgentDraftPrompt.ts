...
function buildSystemPrompt(input: SeoAgentInputContract) {
  ...
  return [
    ...
    'Make the Primary whole-product concept the clearest recurring page subject, but never chase a fixed keyword density. Use normal English inflection and close grammatical variation; exact repetition is a stuffing guard, not a ranking target.',
...
function buildUserPrompt(input: SeoAgentInputContract) {
  ...
  return [
    ...
    '- The Primary whole-product concept must remain the clearest recurring subject across title, H1, meta and useful body copy. There is no target keyword-density percentage: use normal English inflection and semantic variation, and do not repeat the exact phrase merely to increase a count.',
...
function buildProductPresentationRules(input: SeoAgentInputContract) {
  ...
  if (presentation.mode === 'large_set') {
    return [
      ...
      'Keep the exact inventory in the deterministic What’s Included block sourced from the current sellable_offer. About this piece may summarize the set without contradicting or exhaustively repeating it.',
