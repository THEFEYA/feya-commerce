...
function buildSystemPrompt(input: SeoAgentInputContract) {
  ...
  return [
    ...
    'Generate a real main PDP description, not only the right panel. Required left_description blocks: about_this_piece, why_youll_love_it, ideal_for, whats_included. They are the main product text below/near the images.',
    ...
    'Do not render product-level FAQ by default. The top-level faq array should normally be empty or review-only global FAQ suggestions. Product-specific buyer concerns must be handled in PDP blocks, not duplicated as FAQ inside the product tile.',
    'PDP mapping is required: left_description for main description blocks; right_info_panel for compact info blocks only: sizing_fit, production_timing, shipping_delivery, material, care, customization, returns_exchanges, handmade_variation.',
    'Material and care must be separate blocks. Material describes what the product is made from and why it feels/holds shape well. Care describes how easy it is to clean and store. Do not merge them into one weak Materials & care block.',
    'Shipping and production must be separate blocks. Production is 3-5 business days. Shipping is standard 10-14 business days and express 6-9 business days. Do not say if available.',
    'Sizing wording must be confident: use the product size chart, most pieces adjust with straps, exact measurements do not need to be perfect, and custom measurements can be left in order notes or sent to the manager for special fit.',
    'Care wording must be practical and positive: easy to clean by hand with alcohol wipes or mild cleaning products; prefer not to machine wash; store carefully/hanging and avoid long-term heavy pressure so the item keeps shape and lasts for years.',
    ...visionRules,
...
function buildUserPrompt(input: SeoAgentInputContract) {
  ...
  return [
    ...
    '- pdp_blocks: generate the real main product description and right info panel. Do not leave the main description empty.',
    '',
    'Required pdp_blocks:',
    ...
    '- whats_included / left_description: components from variations/source data. Do not tell buyer to clarify standard contents with manager.',
    '- sizing_fit / right_info_panel: size chart + adjustable straps + custom measurements/order notes.',
    '- production_timing / right_info_panel: production 3-5 business days + priority production discussion for a specific date.',
    '- shipping_delivery / right_info_panel: standard 10-14 business days + express 6-9 business days. Never say if available.',
    '- material / right_info_panel: vegan/faux leather with glossy mirror/metallic coating when product facts support it; soft body-facing side; reinforced/doubled structure/shape retention.',
    '- care / right_info_panel: easy hand cleaning with alcohol wipes or mild cleaning products; prefer not to machine wash; store carefully/hanging; avoid long-term heavy pressure.',
    '- customization / right_info_panel: sizing is separate; customization means color/detail/length/coverage/combinations/custom design within TheFEYA style.',
    '- returns_exchanges / right_info_panel: short policy-link wording only.',
    '- handmade_variation / right_info_panel: small natural handmade differences, not alarming.',
    '',
    ...doctrineLines,
    'Visual/style rules:',
