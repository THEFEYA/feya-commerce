...
function buildSystemPrompt(input: SeoAgentInputContract) {
  ...
  const componentRules = [
    ...
    'product.source_variations and product.option_price_rows are evidence about selectable configurations and prices, not permission to mention a price in SEO copy.',
    'product.source_description_fragment may clarify composition, but it must not override configuration rows or component review blockers.',
    'Raw variation labels may be multilingual. Preserve their meaning, but write natural English en-US customer copy.',
...
function buildUserPrompt(input: SeoAgentInputContract) {
  ...
  return [
    ...
    '- Real generation requires product.product_truth_source = seo_product_truth_v1.',
    '- Use product.included_components / product.known_components for confirmed component families.',
    ...
    '- Read product.source_variations, product.option_price_rows, and product.source_description_fragment together. Configuration evidence outranks marketing wording.',
    '- Preserve the meaning of raw labels such as Top Only, Skirt Only, Full Set, One Shoulder, or Full Shoulders, while writing natural English.',
    '- Never infer included pieces from keyword_roles, image styling, manual_focus, or collection terms.',
    '- If product.component_review_blockers or product.unresolved_component_facts contains any item, do not invent What’s included. Return needs_review or blocked with generation_notes.',
    '- If the buyer can choose separate pieces or a full set, say that clearly. Do not imply the full set is included in every purchase or at the lowest price.',
    '',
    ...
    '- whats_included / left_description: confirmed components and selectable configurations from product truth. This comes immediately after the opening.',
...
export function promptGuardrails(input?: SeoAgentInputContract) {
  return [
    ...
    'Real generation requires the versioned seo_product_truth_v1 evidence contract.',
    'Included components must come from component/configuration evidence, never from keyword text or visual styling.',
    'Selectable configurations must not be described as if all pieces are included in every purchase.',
    ...
    'No fake metrics: volume, competition, and trend numbers must come from validated sources with provenance and freshness.',
