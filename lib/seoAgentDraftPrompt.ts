...
function buildSystemPrompt(input: SeoAgentInputContract) {
  ...
  return [
    ...
    'why_youll_love_it must contain 2-4 concise purchase reasons. Include one studio-design value and at least one different practical or visual outcome supported by Product Truth or explicit visual evidence. Two honest reasons are correct when a third independent benefit is not evidenced.',
    ...
    'The fixed right panel already states raw fit, material, production, shipping and care facts. Never copy or lightly paraphrase those sentences into the left description. A verified fact may appear once only when translated into a distinct, concrete buyer outcome.',
...
function buildUserPrompt(input: SeoAgentInputContract) {
  ...
  return [
    ...
    '- about_this_piece / left_description: buyer-job-first opening story. Start with the complete approved event or style look, then explain why this product is useful or distinctive there. Do not enumerate or paraphrase the component inventory: composition is already rendered deterministically in the configuration and What’s Included. A component term may appear only when it supports a new, concrete buyer benefit, never in “combines/pairs/brings together/includes X and Y” wording. Include a supported material, finish, feel, fit, wear or design value only when it answers a real buyer question. Never report left/right position or what is visible from which angle. Do not use TheFEYA here.',
    '- why_youll_love_it / left_description: 2-4 non-duplicative purchase reasons. Exactly one may express studio design/authorship; every other bullet needs a different practical or visual outcome supported by Product Truth or explicit visual evidence. Never invent a third reason merely to fill the block. Do not use TheFEYA here.',
