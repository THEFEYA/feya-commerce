...
export function buildSeoMetricImportContract(keywords: string[]): SeoMetricImportContract {
  ...
  return {
    ...
    sources: [
      { id: 'google_ads', label: 'Google Ads API', role: 'Источник оценок Google demand; competition относится к рекламодателям.', trust: 'primary', allowedForScoring: true },
    ...
    requiredColumns: [
      ...
      { key: 'competition', label: 'Конкуренция', required: true, purpose: 'Рекламная конкуренция; не сложность органического SEO.' },
      { key: 'last_checked', label: 'Дата проверки', required: true, purpose: 'Не использовать старые метрики как свежие.' },
    ],
    optionalColumns: [
      { key: 'low_bid', label: 'Нижняя ставка', required: false, purpose: 'Историческая ставка рекламного аукциона в исходной валюте.' },
      { key: 'high_bid', label: 'Верхняя ставка', required: false, purpose: 'Историческая ставка рекламного аукциона; не конверсия.' },
