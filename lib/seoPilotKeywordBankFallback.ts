...
export async function buildPilotKeywordBankFallbackBundle(productId: string) {
  ...
  const [canonicalResult, focusResult, keywordResult] = await Promise.all([
    supabase
  ...
  if (!product) {
    const reason = focusResult.error?.message || productTruthWarning || 'Pilot product was not found.';
    return failed(`Pilot product read failed: ${reason}`);
  }
  ...
  const diagnostics = {
    mode: canonicalProduct
      ? 'canonical_product_truth_plus_keyword_bank_pilot'
      : 'read_only_canonical_keyword_bank_pilot_fallback',
...
function buildCanonicalComponentTruth(product) {
  ...
  const componentReviewBlockers = collectBlockers(product?.component_review_blockers_json);
...
function collectBlockers(value) {
