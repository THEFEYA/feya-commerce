export type ReleaseConfigurationRepairExecutionRequestRow={
  action_code?:unknown;
...
export function releaseConfigurationRepairApprovalScopeDecision(input:{
  row:ReleaseConfigurationRepairExecutionRequestRow;
  actorUserId:string;
  releaseRef:string;
  productIds:string[];
  ...
  if(row.action_code!=='REPAIR_RELEASE_CONFIGURATION_BINDINGS')reasons.push('ACTION_CODE_MISMATCH');
  ...
  if(payload.contract_version!=='release_configuration_binding_repair_v1')reasons.push('CONTRACT_MISMATCH');
  ...
  if(Number(payload.expected_price_rows)!==856||Number(payload.expected_configuration_axis_rows)!==846
    ||Number(payload.expected_rebind_rows)!==631||Number(payload.expected_create_configurations)!==631)reasons.push('ROW_COUNT_MISMATCH');
