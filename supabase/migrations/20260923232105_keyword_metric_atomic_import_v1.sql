-- Dormant additive contract. Production activation requires legacy-reader reconciliation.
begin;
alter table public.feya_commerce_seo_keyword_metric_import_staging_v1
  add column demand_observation_key text,
  add column demand_evidence_json jsonb,
  add constraint feya_metric_staging_observation_unique unique(demand_observation_key),
  add constraint feya_metric_staging_evidence_pair check (
    (demand_observation_key is null and demand_evidence_json is null) or
    (demand_observation_key is not null and demand_evidence_json is not null and demand_observation_key ~ '^[a-f0-9]{64}$' and jsonb_typeof(demand_evidence_json)='object'));
alter table public.feya_commerce_seo_keyword_metric_snapshots_v1
  add column demand_observation_key text,
  add column demand_evidence_json jsonb,
  add constraint feya_metric_snapshot_observation_unique unique(demand_observation_key),
  add constraint feya_metric_snapshot_staging_fk foreign key(demand_observation_key)
    references public.feya_commerce_seo_keyword_metric_import_staging_v1(demand_observation_key),
  add constraint feya_metric_snapshot_evidence_pair check (
    (demand_observation_key is null and demand_evidence_json is null) or
    (demand_observation_key is not null and demand_evidence_json is not null and demand_observation_key ~ '^[a-f0-9]{64}$' and jsonb_typeof(demand_evidence_json)='object'));

create table public.feya_commerce_seo_metric_import_receipts_v1 (
...
alter table public.feya_commerce_seo_metric_import_receipts_v1 enable row level security;
revoke all on public.feya_commerce_seo_metric_import_receipts_v1 from public,anon,authenticated,service_role;
grant select,insert on public.feya_commerce_seo_metric_import_receipts_v1 to service_role;

create function public.feya_metric_import_immutable_v1() returns trigger
language plpgsql security invoker set search_path='' as $$
begin
  if old.demand_observation_key is not null or (tg_op='UPDATE' and new.demand_observation_key is not null) then
    raise exception using errcode='23514',message='Imported demand observations are immutable; create a new source revision.';
  end if;
  if tg_op='DELETE' then return old; end if;
  return new;
end $$;
create trigger feya_metric_staging_immutable before update or delete on public.feya_commerce_seo_keyword_metric_import_staging_v1
  for each row execute function public.feya_metric_import_immutable_v1();
create trigger feya_metric_snapshot_immutable before update or delete on public.feya_commerce_seo_keyword_metric_snapshots_v1
  for each row execute function public.feya_metric_import_immutable_v1();
revoke all on function public.feya_metric_import_immutable_v1() from public,anon,authenticated,service_role;

create function public.feya_commerce_keyword_metric_import_contract_v1() returns text
language sql stable security invoker set search_path='' as $$ select 'atomic_keyword_metric_import_v1'::text $$;
revoke all on function public.feya_commerce_keyword_metric_import_contract_v1() from public,anon,authenticated;
grant execute on function public.feya_commerce_keyword_metric_import_contract_v1() to service_role;

create function public.feya_commerce_import_keyword_metrics_atomic_v1(p_request_key text,p_payload jsonb)
...
revoke all on function public.feya_commerce_import_keyword_metrics_atomic_v1(text,jsonb) from public,anon,authenticated;
grant execute on function public.feya_commerce_import_keyword_metrics_atomic_v1(text,jsonb) to service_role;
commit;
