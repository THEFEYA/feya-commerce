...
### Checkpoint 2026-09-24 — предметная сверка и design review
...
## Owner scope update — 24 сентября 2026

Decision `owner-launch-scope-20260924-02` и `Initial_Launch_Pilot_20260924.md` меняют зависимость запуска: обработка всех 243 карточек, 19 идентифицированных отложенных корсетов/зеркальных комплектов и всех девяти model pairs не является условием ограниченного выпуска. Подтверждённые изделия с пластиком и ранее подготовленные корсеты иных конструкций не исключаются по материалу. Пять конкретных product/page/draft IDs выбраны для preflight из 28 совместимых по сохранённым источникам; ни один из этих статусов не открывает публикацию, покупку или индексацию.

Состав №5 уточнён: бра + юбка, бра из золотой vegan leather. Остальные неподтверждённые поля не заполнены по догадке. Dress/Jacket отсутствуют в текущем UI/server allowlist; их поддержка и отдельная mirror-clothing категория остаются expansion work, без автоматического создания indexable pages. 23 группы повторного Primary в approved drafts требуют scoped ownership review; это не GSC measurement.

## Полный approved catalog — owner clarification 24 сентября

`owner-full-approved-catalog-20260924-03` уточняет цель: все 208 одобренных карточек входят в целевой каталог; прежняя пятёрка — интеграционная выборка. Прежний strict preflight из 28 не ограничивает release scope. Проверены все 208 saved outputs: отсутствуют точные дубли meta descriptions и полных описаний; 64 карточки затронуты точными совпадениями title/H1. Массовая перегенерация не обоснована. Подготовлены 208 version-bound projections в существующий PDP component; public route ещё не подключён.

Для подтверждённого Bra/Skirt добавлен exact-ID correction без изменения цен и identity. 208/208 целевых составов теперь разрешаются кодом, что не доказывает purchasability. Текущий маршрут C1–C6 и данные: `Approved_Catalog_Launch_20260924.md`. Старые approval/metric snapshots не сбрасываются; отсутствие нового API доступа не является поводом переделывать весь контент.
