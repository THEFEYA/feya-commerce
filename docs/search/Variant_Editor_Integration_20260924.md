...
Существующая Product OS карточка `/admin/products/{slug-or-id}` получила дополнительный блок «Варианты товара». Он включается только серверным `FEYA_PRODUCT_VARIANT_DRAFT_ENABLED=true` при обязательном Auth. Оформление окружающей карточки, storefront, шрифты, цвета, CSS и прежние поля не изменены. Карточка фактов в Growth `/admin/product-facts-review` показывает сохранённую revision того же товара через тот же API, с переходом в Product OS для редактирования. Второго хранилища или отдельного writer в Growth нет.
