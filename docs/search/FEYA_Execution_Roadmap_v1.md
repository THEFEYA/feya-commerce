...
## Текущий checkpoint: ответы C1 закрыты; следующий пакет C4.1, 24.09.2026
...
C4: schema/read-only audit и реализованный pure variant contract в `commerceVariantContract.ts`: stable tuple IDs, no Cartesian/fallback pricing, revision conflict, подтверждённая exact цена, запрет переназначения/удаления истории. **DB writer/RPC и UI сохранения цветов ещё не подключены.** Следующий C4.1 пакет: additive schema + атомарный Product OS writer с actor validation, CAS/idempotency и execution/change event; проверить на изолированном Postgres. Затем C4.2 save/reload в существующей админке → C3.2 единый immutable release/Shop cohort → C4.3 quote/checkout и crawl → C6 pre-index. C5 company/domain/returns продолжать независимо. Не обещать автоматическое обновление sitemap на admin draft save; публикация и производные страницы используют общий release.

Предшествующий `ddc58573c6d5cdd91d956ebdf5d6a73f4125b78c` проверен: CI run `36022158818`, 9/9 jobs, 36/36 runtime, 208/208 approved server HTML. Новый пакет проверять по точному head PR; результат CI фиксировать в PR checkpoint, не переносить статус старого commit. **От владельца сейчас ничего не требуется: ни повторных ответов, ни новых исследований, файлов или подключений.**
