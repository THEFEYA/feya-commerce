-- FEYA Commerce / Product Truth v3 read-only preview
-- SELECT only. No CREATE, ALTER, GRANT, REVOKE, INSERT, UPDATE or DELETE.
-- This preview must pass before the proposed view is considered for application.

WITH product_truth_preview AS (
WITH
product_focus AS (
    SELECT
...
SELECT jsonb_build_object(
    'row_count', count(*),
    'distinct_product_count', count(DISTINCT canonical_product_id),
    'duplicate_product_count', count(*) - count(DISTINCT canonical_product_id),
    'ready_product_count', count(*) FILTER (
        WHERE jsonb_array_length(included_components) > 0
          AND jsonb_array_length(unresolved_component_facts) = 0
          AND jsonb_array_length(component_review_blockers_json) = 0
    ),
    'alignment_required_unresolved_count', count(*) FILTER (
        WHERE COALESCE((component_evidence->'variation_price_alignment'->>'alignment_required')::boolean, false)
          AND NOT COALESCE((component_evidence->'variation_price_alignment'->>'alignment_resolved')::boolean, false)
    ),
    'non_configuration_optional_row_count', COALESCE(sum((
        SELECT count(*)
        FROM jsonb_array_elements(optional_configurations) item
        WHERE COALESCE(item->>'detected_canonical_axis', '') <> 'configuration'
    )), 0),
    'ready_candidates', COALESCE(jsonb_agg(
        jsonb_build_object(
            'canonical_product_id', canonical_product_id,
            'matched_etsy_listing_id', matched_etsy_listing_id,
    ...
    'review_candidates', COALESCE((
        SELECT jsonb_agg(candidate_row ORDER BY candidate_row->>'product_slug')
        FROM (
            SELECT jsonb_build_object(
                'canonical_product_id', p.canonical_product_id,
                'matched_etsy_listing_id', p.matched_etsy_listing_id,
                'product_slug', p.product_slug,
                'optional_configuration_count', jsonb_array_length(p.optional_configurations),
                'component_coverage_count', jsonb_array_length(COALESCE(
                    p.component_evidence->'component_configuration_coverage',
                    '[]'::jsonb
                )),
                'structural_blocker_count', (
                    SELECT count(*)
                    FROM jsonb_array_elements(p.component_review_blockers_json) blocker
                    WHERE blocker->>'reason' IN (
                        'missing_component_phrase_mapping',
                        'option_component_family_null',
                        'configuration_component_family_null',
                        'source_configuration_mismatch',
                        'missing_sellable_configuration'
                    )
                ),
                'unresolved_fact_count', jsonb_array_length(p.unresolved_component_facts),
                'review_blocker_count', jsonb_array_length(p.component_review_blockers_json)
            ) AS candidate_row
            FROM product_truth_preview p
            WHERE jsonb_array_length(p.optional_configurations) > 0
            ...
            ORDER BY
                ...
                jsonb_array_length(p.component_review_blockers_json),
                jsonb_array_length(p.unresolved_component_facts),
                p.product_slug
            LIMIT 10
        ) ranked
    ), '[]'::jsonb),
    'pilot', COALESCE((
        jsonb_agg(jsonb_build_object(
            ...
            'matched_etsy_listing_id', matched_etsy_listing_id,
            'included_components', included_components,
            'optional_configurations', optional_configurations,
            'unresolved_component_facts', unresolved_component_facts,
            'component_review_blockers_json', component_review_blockers_json
        )) FILTER (WHERE matched_etsy_listing_id = '4348580005')
    )->0, '{}'::jsonb)
) AS product_truth_preview_result
FROM product_truth_preview;
